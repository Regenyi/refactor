...
def input_validation(choosen_number):
    while not str.isdigit(choosen_number) or (int(choosen_number) > 9 or int(choosen_number) < 1):
        choosen_number = input("Enter the number2: ")
...
def check_full_board(filled_board, original, exit):
    global choosen_sudoku
    global choosen_row
    global choosen_column
    while not filled_board and not exit: # amig ezek false-ok
        draw_board()
        exit = False
        ...
        for i in range(2):  # ez lenne a main input???
            ...
            if is_restart(value):
                ...
                restart = True
                break
            value = row_column_validation(value, input_matrix, i)
            if exit or restart:
                break
        ...
        if not restart:
            ...
            if restart:
                continue
            check_cell(choosen_number)


def check_cell(choosen_number):
    global original
    global filled_board
    global choosen_row
    global choosen_column
    int_choosen_row = int(choosen_row) - 1
    int_choosen_column = int(choosen_column) - 1

    if original[int_choosen_row][int_choosen_column] == " ":
        if choosen_number == " ":
            choosen_sudoku[int_choosen_row][int_choosen_column] = choosen_number
        else:
            choosen_sudoku[int_choosen_row][int_choosen_column] = int(choosen_number)
    else:
        if choosen_number != " ":
    ...
    os.system('clear')
    filled_board = check_space_in_board()  # nyerés ellenőrzése
    check_win(filled_board)


def check_win(filled_board):
    global choosen_sudoku
    if filled_board:
...
def main():
    ...
    if not quit:
        ...
        exit = False
        check_full_board(filled_board, original, exit) 
        main()
