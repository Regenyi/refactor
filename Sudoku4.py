...
def menu():
    ...
    while True:  # amig igaz 
        ...
        print()
        # level selection part: -> maybe move it to a separate funciton? the while loop makes it hard  
...
def input_correction(choosen_number):
...
def write_to_cell(choosen_number):
    ...
    if game_data.original[int_choosen_row][int_choosen_column] == " ": 
        if choosen_number == " ": #delete a number
    ...
    os.system('clear')
    game_data.filled_board = check_if_board_full()  # sajna a game cycle-nek ez kellett, különben elég lenne itt?
    ...
    while not game_data.filled_board and not exit:  # amig ezek false-ok
        ...
        for i in range(2):  # main input
            ...
            if exit or game_data.restart:
                break
            if game_data.input_matrix[i] == "row": # itt kilóg a lóláb, ez valami kivétel kezelése?
                game_data.choosen_row = value
            else:
                game_data.choosen_column = value # ennyi a különbség : _column ezt nem lehet az i-re kötni? 
        if exit:
            break

        if not game_data.restart: # ha eljut idáig, akkor jöhet a tényleges sudoku szám 
            ...
            if (choosen_number == "" or choosen_number == "0"):
                ...
                choosen_number = input_correction(choosen_number) # input type beazonositás/eset kiválasztás
            if exit:
                break
            if game_data.restart:
                continue
            write_to_cell(choosen_number)
...
def main():
    ...
    if not game_data.quit:
        game_data.original = deepcopy(game_data.choosen_level) # ez nem fog felülirni valamit a második main lefutásakor? 
...
# enter number validáció és validációk egyszerűsitése
# bug in the original: exit után új pálya nem működik, régit tölti be


# kérdések:

# level selection part to function in menu
# konstansok bemozgatása a game_data-ba?
# class inicializálás helye (game_data = GameData())
# ha nem class, hogyan számolod fel a listákat, többit?
# ez igy jobb, könnyebb olvasnia bárkinek?
# kesze kusza 
